...
from app.services.ai_client import AIClient, classify_openai_error, MAX_RETRIES
...
# ============================================================
# 1. classify_openai_error 异常分类测试
# ============================================================

class TestClassifyOpenAIError(unittest.TestCase):
    """测试 classify_openai_error 将各类异常正确分类"""
...
# ============================================================
# 2. TaskRunner._process_file_with_retry 文件级重试测试
# ============================================================
...
class TestProcessFileWithRetry(unittest.TestCase):
    ...
    def _make_runner(self, file_processor):
        ...
        failed_svc = MagicMock(spec=FailedRecordService)
        return TaskRunner(state=state, file_processor=file_processor, failed_record_service=failed_svc)
...
class TestCallAIRetry(unittest.TestCase):
    ...
    def test_success_on_first_call(self, mock_ws):
        ...
        ai_client = AIClient(ProcessingState())
    ...
    def test_unclassified_error_classified_as_non_retryable(self, mock_ws):
        """未分类异常 → classify_openai_error 判定为不可重试 → 直接抛出"""
        mock_ws.return_value = None
        mock_client = MagicMock()
...
# ============================================================
# 6. FileProcessor.process_file 异常分类集成测试
# ============================================================
...
class TestProcessFileErrorClassification(unittest.TestCase):
    ...
    def test_retryable_error_returns_retryable(self, mock_read):
        """process_file 捕获 RetryableError → retryable=True"""
        mock_read.return_value = "content"
        mock_ai_client = MagicMock(spec=AIClient)
        mock_ai_client.call.side_effect = NetworkError("网络断开")
        fp = FileProcessor(mock_ai_client)
        result = fp.process_file("test.txt", None, None, None)
        self.assertTrue(result["retryable"])
        self.assertIn("error", result)

    @patch('app.services.file_processor.read_file_with_encoding')
    def test_provider_error_returns_non_retryable(self, mock_read):
        """process_file 捕获 ProviderError → retryable=False"""
        mock_read.return_value = "content"
        mock_ai_client = MagicMock(spec=AIClient)
        mock_ai_client.call.side_effect = ProviderError("API Key无效")
        fp = FileProcessor(mock_ai_client)
        result = fp.process_file("test.txt", None, None, None)
        self.assertFalse(result["retryable"])
        self.assertIn("error", result)
    ...
    def test_file_processing_error_returns_non_retryable(self, mock_read):
        """process_file 捕获 FileProcessingError → retryable=False"""
    ...
    def test_success_returns_output(self, mock_read):
        """process_file 成功 → 返回 output"""
        mock_read.return_value = "content"
        mock_ai_client = MagicMock(spec=AIClient)
        mock_ai_client.call.return_value = "AI summary"
        fp = FileProcessor(mock_ai_client)
        with patch.object(FileProcessor, 'save_response', return_value="test.md"):
            result = fp.process_file("test.txt", None, None, None)
            self.assertEqual(result["output"], "test.md")
            self.assertNotIn("error", result)
